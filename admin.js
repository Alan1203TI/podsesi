...
import { getFirestore, collection, addDoc, deleteDoc, doc, setDoc, onSnapshot, query, orderBy, serverTimestamp } from 'https://www.gstatic.com/firebasejs/10.12.5/firebase-firestore.js';
...
function startListeners(){
  onSnapshot(query(collection(db,CATEGORIES_COLLECTION), orderBy('name')), snap => { categories = snap.docs.map(d=>({id:d.id,...d.data()})); renderCategories(); fillCategorySelect(); });
  onSnapshot(query(collection(db,EPISODES_COLLECTION), orderBy('createdAt','desc')), snap => { episodes = snap.docs.map(d=>({id:d.id,...d.data()})); renderEpisodes(); });
}
$('addCategoryBtn').onclick = async () => { const name=$('categoryName').value.trim(); if(!name) return; await addDoc(collection(db,CATEGORIES_COLLECTION), { name, createdAt: serverTimestamp() }); $('categoryName').value=''; };
function renderCategories(){ $('categoriesList').innerHTML = categories.map(c => `<div class="manage-item"><strong>${esc(c.name)}</strong><button class="danger-btn" data-delcat="${c.id}">Excluir</button></div>`).join(''); document.querySelectorAll('[data-delcat]').forEach(b => b.onclick = async()=>{ if(confirm('Excluir esta categoria? Os episódios não serão apagados.')) await deleteDoc(doc(db,CATEGORIES_COLLECTION,b.dataset.delcat)); }); }
...
$('episodeForm').onsubmit = async (e) => { e.preventDefault(); const audio=$('audioFile').files[0]; if(!audio) return alert('Selecione o arquivo de áudio.'); const cover=$('coverFile').files[0]; const id = Date.now().toString(); $('saveEpisodeBtn').disabled=true; try{
  ...
  await addDoc(collection(db,EPISODES_COLLECTION), { title:$('title').value.trim(), description:$('description').value.trim(), category:$('categorySelect').value, audioUrl, coverUrl, audioPath, coverPath, plays:0, createdAt: serverTimestamp() });
...
async function deleteEpisode(id){ const ep=episodes.find(e=>e.id===id); if(!confirm('Excluir este episódio e seus arquivos?')) return; await deleteDoc(doc(db,EPISODES_COLLECTION,id)); if(ep?.audioPath) deleteObject(ref(storage,ep.audioPath)).catch(()=>{}); if(ep?.coverPath) deleteObject(ref(storage,ep.coverPath)).catch(()=>{}); }
